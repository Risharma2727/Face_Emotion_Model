...
# 🎙 Load Audio Emotion Model (Keras)
...
# 🎭 Load TFLite Facial Emotion Model
...
def hybrid_fusion(face_probs, audio_probs):
    # Feature-level fusion (simple concatenation)
    ...
    # Optional softmax recalibration
...
def predict_audio_emotion(audio_input):
    with tf.device('/GPU:0'):
        return audio_model.predict(audio_input)
...
def record_audio_and_predict():
    ...
    try:
        ...
        audio_probs = predict_audio_emotion(audio_input)[:, :7]
...
while True:
    ...
    if len(faces) > 0:
        ...
        face_probs = predict_face_emotion_tflite(roi)
    ...
    combined = hybrid_fusion(face_probs, audio_probs)
    ...
    cv2.imshow("Multimodal Emotion Detection (Hybrid Fusion)", display_frame)
